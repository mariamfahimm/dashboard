...
// Run with: mongosh "YOUR_MONGODB_CONNECTION_STRING" < scripts/setupDatabase.mongodb.js
// Or copy-paste into MongoDB Compass or Atlas UI
// Replace YOUR_MONGODB_CONNECTION_STRING with your actual MongoDB URI from .env file
...
db.users.insertMany([
  {
    ...
    password: '$2b$10$example_hashed_password_teacher1',
    role: 'teacher',
    createdAt: new Date(),
    updatedAt: new Date()
  },
...
db.grades.createIndex({ enrollmentId: 1, assignmentId: 1 }, { unique: true })
db.grades.createIndex({ studentId: 1 })
db.grades.createIndex({ courseId: 1 })

db.alerts.createIndex({ studentId: 1, timestamp: -1 })
db.alerts.createIndex({ read: 1 })

db.recommendations.createIndex({ studentId: 1, createdAt: -1 })
db.recommendations.createIndex({ accepted: 1, dismissed: 1 })
